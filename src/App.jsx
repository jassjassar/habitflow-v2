...
// ── 6 THEMES ──────────────────────────────────────────────────────────────
const THEMES = {
  dark:   { name:"Dark Mode",     emoji:"🌑", gold:"#C9A84C", gold2:"#E8C96A", bg:"#08080D", bg2:"#0F0F16", bg3:"#15151F", border:"#1E1E2A", text:"#F0EDE8", muted:"#666", accent:"#7c6af7", isDark:true },
  light:  { name:"Light Mode",    emoji:"☀️",  gold:"#B8922A", gold2:"#C9A84C", bg:"#F8F6F1", bg2:"#FFFFFF", bg3:"#F0EDE5", border:"#E0DDD6", text:"#1A1814", muted:"#999", accent:"#7c6af7", isDark:false },
  mint:   { name:"Fresh Mint",    emoji:"🌿", gold:"#10b981", gold2:"#34d399", bg:"#f0fdf6", bg2:"#ffffff", bg3:"#ecfdf5", border:"rgba(16,185,129,0.2)", text:"#111827", muted:"#9ca3af", accent:"#06b6d4", isDark:false },
  aurora: { name:"Dark Aurora",   emoji:"🌌", gold:"#7c6af7", gold2:"#a78bfa", bg:"#0f0e1a", bg2:"#1e1c35", bg3:"#252340", border:"rgba(124,106,247,0.2)", text:"#ffffff", muted:"#6b6b8a", accent:"#06b6d4", isDark:true },
  ocean:  { name:"Ocean Deep",    emoji:"🌊", gold:"#0ea5e9", gold2:"#38bdf8", bg:"#0c1929", bg2:"#132740", bg3:"#163049", border:"rgba(14,165,233,0.2)", text:"#f0f9ff", muted:"#5b8fa8", accent:"#06b6d4", isDark:true },
  coral:  { name:"Sunset Coral",  emoji:"🌅", gold:"#f97316", gold2:"#fb923c", bg:"#fff8f5", bg2:"#ffffff", bg3:"#fff0ea", border:"rgba(249,115,22,0.2)", text:"#1c0a00", muted:"#9a6b50", accent:"#ef4444", isDark:false },
  rose:   { name:"Rose Gold",     emoji:"🌸", gold:"#e11d48", gold2:"#fb7185", bg:"#fff5f7", bg2:"#ffffff", bg3:"#ffeef2", border:"rgba(225,29,72,0.18)", text:"#1a0010", muted:"#9f6070", accent:"#c084fc", isDark:false },
  slate:  { name:"Midnight Slate",emoji:"🌙", gold:"#6366f1", gold2:"#818cf8", bg:"#0f172a", bg2:"#1e293b", bg3:"#263245", border:"rgba(99,102,241,0.2)", text:"#f1f5f9", muted:"#64748b", accent:"#22d3ee", isDark:true },
};
...
const css = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .t-dark{--bg:#08080D;--bg2:#0F0F16;--bg3:#15151F;--border:#1E1E2A;--text:#F0EDE8;--muted:#666;--gold:#C9A84C;--gold2:#E8C96A;}
  .t-light{--bg:#F8F6F1;--bg2:#FFFFFF;--bg3:#F0EDE5;--border:#E0DDD6;--text:#1A1814;--muted:#999;--gold:#B8922A;--gold2:#C9A84C;}
  .t-mint{--bg:#f0fdf6;--bg2:#ffffff;--bg3:#ecfdf5;--border:rgba(16,185,129,0.2);--text:#111827;--muted:#9ca3af;--gold:#10b981;--gold2:#34d399;}
  .t-aurora{--bg:#0f0e1a;--bg2:#1e1c35;--bg3:#252340;--border:rgba(124,106,247,0.2);--text:#ffffff;--muted:#6b6b8a;--gold:#7c6af7;--gold2:#a78bfa;}
  .t-ocean{--bg:#0c1929;--bg2:#132740;--bg3:#163049;--border:rgba(14,165,233,0.2);--text:#f0f9ff;--muted:#5b8fa8;--gold:#0ea5e9;--gold2:#38bdf8;}
  .t-coral{--bg:#fff8f5;--bg2:#ffffff;--bg3:#fff0ea;--border:rgba(249,115,22,0.2);--text:#1c0a00;--muted:#9a6b50;--gold:#f97316;--gold2:#fb923c;}
  .t-rose{--bg:#fff5f7;--bg2:#ffffff;--bg3:#ffeef2;--border:rgba(225,29,72,0.18);--text:#1a0010;--muted:#9f6070;--gold:#e11d48;--gold2:#fb7185;}
  .t-slate{--bg:#0f172a;--bg2:#1e293b;--bg3:#263245;--border:rgba(99,102,241,0.2);--text:#f1f5f9;--muted:#64748b;--gold:#6366f1;--gold2:#818cf8;}
  ...
  .theme-opt:hover{transform:scale(1.04);transition:transform .2s}
...
export default function HabitFlow() {
  ...
  const applyThemeKey = (key) => {
    setThemeKey(key);
    localStorage.setItem("hf_themekey", key);
    localStorage.setItem("hf_theme_manual", "true");
    const th = THEMES[key];
    if (th) { setTheme(th.isDark ? "dark" : "light"); localStorage.setItem("hf_theme", th.isDark ? "dark" : "light"); }
    setShowThemePicker(false);
  };

  useEffect(() => {
    const interval = setInterval(() => {
      if (!localStorage.getItem("hf_theme_manual")) setTheme(getTimeTheme());
    }, 60000);
...
Be encouraging, specific, and concise. Give actionable advice. Keep responses under 150 words.`;
    ...
    <div style={S.wrap} className={isLight?"light":""}>
      ...
      <nav style={S.nav}>
        ...
        <div style={{display:"flex",gap:10,alignItems:"center"}}>
          <button onClick={()=>setShowThemePicker(true)} style={{...S.btn,...S.ghostBtn,padding:"8px 14px"}}>🎨 Theme</button>
  ...
  return (
    <div style={S.wrap} className={"t-"+themeKey+(isLight?" light":"")}>
      ...
      {/* THEME PICKER MODAL */}
      {showThemePicker && (
        <div onClick={()=>setShowThemePicker(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",zIndex:2000,display:"flex",alignItems:"flex-end",backdropFilter:"blur(5px)"}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"var(--bg2)",borderRadius:"24px 24px 0 0",padding:"24px 18px 36px",width:"100%",borderTop:"1px solid var(--border)"}}>
            <div style={{fontSize:18,fontWeight:800,marginBottom:6,color:"var(--text)"}}>🎨 Choose Theme</div>
            <div style={{fontSize:13,color:"var(--muted)",marginBottom:18}}>8 themes to match your vibe</div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
              {Object.entries(THEMES).map(([key,th])=>(
                <button key={key} onClick={()=>applyThemeKey(key)} className="theme-opt" style={{padding:0,background:"transparent",border:`2.5px solid ${themeKey===key?"var(--gold)":"transparent"}`,borderRadius:16,overflow:"hidden",cursor:"pointer"}}>
                  <div style={{background:th.bg,padding:10,height:56,display:"flex",flexDirection:"column",gap:4}}>
                    <div style={{display:"flex",gap:3}}>
                      <div style={{width:8,height:8,borderRadius:"50%",background:th.gold}}/>
                      <div style={{width:8,height:8,borderRadius:"50%",background:th.gold2}}/>
                    </div>
      ...
      {/* MOOD PICKER */}
      {showMoodPicker && (
        <div onClick={()=>setShowMoodPicker(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:2000,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
          <div onClick={e=>e.stopPropagation()} style={{...S.card,maxWidth:340,width:"100%",padding:24}} className="fade">
            <div style={{fontSize:17,fontWeight:800,marginBottom:6,color:"var(--text)"}}>😊 How are you feeling?</div>
            <div style={{fontSize:12,color:"var(--muted)",marginBottom:18}}>Tap to log today's mood</div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:8,marginBottom:10}}>
              {["😴","😐","🙂","😊","🔥"].map(m=>(
                <button key={m} onClick={()=>saveMood(m)} style={{padding:"14px 4px",borderRadius:12,border:`2px solid ${todayMood===m?"var(--gold)":"var(--border)"}`,background:todayMood===m?"var(--bg3)":"transparent",fontSize:26,cursor:"pointer",transition:"all .2s"}}>{m}</button>
              ))}
            </div>
            <div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:"var(--muted)",fontWeight:700,marginBottom:14}}>
      ...
      <nav style={S.nav}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          <button onClick={()=>setShowThemePicker(true)} style={{...S.btn,...S.ghostBtn,padding:"7px 12px",fontSize:12}}>🎨 {THEMES[themeKey]?.emoji||"🌑"}</button>
      ...
      <div style={{padding:"12px 24px 0"}}>
        <div style={{background:"var(--bg2)",border:"1px solid var(--border)",borderRadius:16,padding:"14px 18px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              <span style={{fontSize:22}}>💧</span>
              <div>
                <div style={{fontSize:10,color:"var(--muted)",fontWeight:700,textTransform:"uppercase",letterSpacing:.5}}>WATER</div>
                <div style={{fontSize:18,fontWeight:900,color:"var(--text)"}}>{waterCups}<span style={{fontSize:12,color:"var(--muted)",fontWeight:400}}> / {waterGoal} cups</span></div>
              </div>
            </div>
            <div style={{display:"flex",gap:6,alignItems:"center"}}>
              <button onClick={()=>addWaterCup(-1)} style={{...S.btn,background:"var(--bg3)",border:"1px solid var(--border)",color:"var(--muted)",padding:"4px 10px",fontSize:13}}>−</button>
              <button onClick={()=>addWaterCup(1)} style={{...S.btn,...S.goldBtn,padding:"4px 10px",fontSize:13}}>+</button>
            </div>
          </div>
          <div style={{display:"flex",gap:4}}>
            {Array.from({length:waterGoal},(_,i)=>(
              <div key={i} onClick={()=>{const d=new Date().toISOString().slice(0,10);setWaterCups(i+1);localStorage.setItem("hf_water_"+d,i+1);}}
                style={{flex:1,height:20,borderRadius:6,background:i<waterCups?"#06b6d4":"var(--bg3)",border:"1px solid var(--border)",cursor:"pointer",transition:"background .2s"}}/>
            ))}
          </div>
        </div>
      </div>
      ...
      {/* MOOD CHECK-IN */}
      <div style={{padding:"12px 24px 0"}}>
        <button onClick={()=>setShowMoodPicker(true)} style={{...S.btn,width:"100%",background:"var(--bg2)",border:"1px solid var(--border)",borderRadius:16,padding:"13px 18px",display:"flex",alignItems:"center",justifyContent:"space-between",color:"var(--text)"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <span style={{fontSize:20}}>😊</span>
            <div style={{textAlign:"left"}}>
              <div style={{fontSize:10,color:"var(--muted)",fontWeight:700,textTransform:"uppercase",letterSpacing:.5}}>MOOD CHECK-IN</div>
              <div style={{fontSize:14,fontWeight:700}}>{todayMood || "How are you feeling today?"}</div>
            </div>
          </div>
          <span style={{color:"var(--gold)",fontSize:18}}>+</span>
        </button>
